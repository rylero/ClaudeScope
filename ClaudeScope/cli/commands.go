...
func runHelp() ([]byte, error) {
	...
	s := schema{
		...
		Commands: []cmd{
			...
			{
				Name:  "query",
				Desc:  "Run a pipe query joining multiple fields on a shared, forward-filled timestamp axis. This is a SUBSET OF SPLUNK SPL: write standard SPL and it works. Supported: where (alias: search), eval <name> = <expr>, rex field=<field> \"<regex>\", stats <agg>(<field>) [as <alias>] [by <field>...], table (alias: fields), sort [-]<field>, head N, tail N, and the ClaudeScope-only 'ranges' (must be last; collapses matching rows into [{start,end}] intervals). Aggregates: avg,min,max,sum,count,median,stdev,p50,p90,p99. eval operators: + - * / and functions abs,round,sqrt,ceil,floor,min,max,pow (put spaces around operators). rex uses (?<name>...) named groups. Comparison ops: > < >= <= == (= also works) != and or NOT. '_time' aliases the Timestamp column. NOT SUPPORTED (errors): timechart, dedup, subsearches, lookup, join, transaction.",
